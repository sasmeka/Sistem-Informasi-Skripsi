...
class Periode extends BaseController
{
    ...
    public function update_data_periode()
    {
        ...
        $data1 = $this->api->get_meta_api("https://api.trunojoyo.ac.id:8212/siakad/v1/periode?page=1&take=100");
        $total_page = intval($data1->pageCount);
        function add_data($page, $a)
        {
            $array_data = [];
            $data = $a->api->get_data_api("https://api.trunojoyo.ac.id:8212/siakad/v1/periode?page=$page&take=100");
            foreach ($data as $key) {
                ...
                // }
                array_push($array_data, "('" . $key->idperiode . "'," . $nama . ",$page)");
            }
            return $array_data;
        }
        // $r = [];
        for ($i = 1; $i <= $total_page; $i++) {
            ...
            $rr = implode(', ', $result);
            $this->db->query("INSERT INTO tb_periode (idperiode,namaperiode,page) VALUES $rr");
